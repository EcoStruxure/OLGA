...
using Olga.Trinity.Attriubtes;</#if><#if Zclass.getListOfImports()?? && preserveNameSpaceOfOntology==true><#list Zclass.getListOfImports() as importedPackage>
...
namespace ${packageName}
{
    ...
    public<#if generatePartial!false> partial</#if> class ${Zclass.getzClassName()}<#if Zclass.getSuperZClassList()?? || motherToExtend??> : <#if motherToExtend??><#if motherToExtend.getzClassName() == "Resource" && preserveNameSpaceOfOntology==true>${motherToExtend.getPackageName()}.${motherToExtend.getzClassName()}<#else>${motherToExtend.getzClassName()}</#if><#else>ContextResource</#if></#if>, I${Zclass.getzClassName()}
    {
        ...
        <#if ObjectPropertyList.isHidingParentProperty()>new </#if>protected readonly PropertyMapping<${rangeName}> _${varName?uncap_first} = new PropertyMapping<${rangeName}>("${varName?cap_first}${rangeName}${Zclass.getzClassName()}", "${ObjectPropertyList.getObjectProperty()}");
        [RdfProperty("${ObjectPropertyList.getObjectProperty()}")]
        <#if ObjectPropertyList.isHidingParentProperty()>new </#if>public ${rangeName} ${varName?cap_first}
        {
